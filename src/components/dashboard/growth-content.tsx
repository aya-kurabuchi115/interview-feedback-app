...
/**
 * 成長記録ページのデータ取得 + レンダリングを行う async Server Component
 * Suspense 境界内で使用する
 */
export async function GrowthContent({ userId }: GrowthContentProps) {
  const supabase = await createClient();

  // 面接データ取得（completed のみ、自分のデータのみ）
  const { data: interviews } = await supabase
    .from("interviews")
    .select(
      "id, title, company_name_snapshot, interview_category, interview_date, created_at, status"
    )
    .eq("user_id", userId)
    .eq("status", "completed")
  ...
  const { data: feedbacks } =
    interviewIds.length > 0
      ? await supabase
          .from("feedbacks")
          .select(
            "interview_id, overall_score, category_scores, filler_words, created_at"
          )
  ...
  const avgFillerRate =
    fillerRatesWithData.length > 0
      ? Math.round(
          (fillerRatesWithData.reduce((a, b) => a + b, 0) /
            fillerRatesWithData.length) *
            10
        ) / 10
      : null;
  ...
  return (
    <>
      ...
      <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <StatsCard label="今月の面接数" value={thisMonthScores.length} />
      </div>
